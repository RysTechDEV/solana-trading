...
const setup = () => {
	return new Promise((resolve) => {
		...
		db.prepare("CREATE TABLE IF NOT EXISTS telegramChannels (userId, username)").run()
...
const getTrades = async (user_id) => {
	...
	for (let i = 0; i < trades.length; i++) {
		...
		const lastIndexForThisItem = trades.map(item => item.address).lastIndexOf(trades[i].address)
		...
		const tokenFoundData = balances.find(
			(balanceItem) => balanceItem.token && trades[i].address && balanceItem.token.toLowerCase() == trades[i].address.toLowerCase(),
		)
		if (!tokenFoundData) {
...
const sellToken = async (id, percentageToSell, lockProfits, user_id) => {
	...
	console.log("Selling token", tradeData.address, "id", id, 'percentageToSell', percentageToSell, 'lockProfits', lockProfits)
	try {
